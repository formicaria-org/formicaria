...
fn a_note_discussion_yields_a_proposal_that_is_reviewed_then_accepted() {
    ...
    commands::reply(
        &mut store,
        &host,
        "@qwen3-4b-2507 tidy these notes and add a summary?",
    )
    .unwrap();
    commands::reply(
        &mut store,
        &host,
        "On it — I've drafted a cleaned-up version. /propose",
    )
    .unwrap();
    ...
    assert!(
        diff.patch.contains("## Summary"),
        "the diff shows the new content: {}",
        diff.patch
    );
    assert!(
    ...
    assert_eq!(
        who, "qwen3-4b-2507",
        "the proposal is attributed to the model that made it"
    );
...
fn accept_proposal_the_gui_button_merges_the_branch_into_main() {
    ...
    assert_eq!(
        commands::accept_proposal(&store, p, &prop.id).unwrap(),
        git::Accepted::Merged
    );
...
fn a_user_iterates_with_the_agent_over_several_rounds_before_accepting() {
    ...
    decline(&v1);
    assert!(
        !commands::proposal_diff(&store, p, &v1).unwrap().exists,
        "the declined v1 is gone"
    );
    ...
    assert_eq!(
        open_count(),
        1,
        "round 2 opens a fresh proposal after the first was declined"
    );
    assert_eq!(
        author_of(&v2),
        "lfm2.5-1.2b",
    ...
    assert!(
        d2.patch.contains("Action items"),
        "v2 now carries the action items: {}",
        d2.patch
    );
    ...
    let merge = git_run(
        p,
        &["merge", "--no-ff", "--no-edit", &format!("proposal/{v3}")],
    );
    assert!(
        merge.status.success(),
    ...
    assert!(
        summary_at < actions_at,
        "the accepted version leads with the summary: {live}"
    );
    assert!(
        live.contains("Arne\n") && !live.contains("Arnne"),
