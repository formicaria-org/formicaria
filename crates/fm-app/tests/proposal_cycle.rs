...
fn a_note_discussion_yields_a_proposal_that_is_reviewed_then_accepted() {
    ...
    commands::reply(&mut store, &host, "@qwen3-4b-2507 tidy these notes and add a summary?").unwrap();
    commands::reply(&mut store, &host, "On it — I've drafted a cleaned-up version. /propose").unwrap();
    ...
    let prop =
        commands::create_proposal(&mut store, p, &host, body, &ProposalLimits::default(), author).unwrap();
    ...
    let merge =
        git_run(p, &["merge", "--no-ff", "--no-edit", &format!("proposal/{}", prop.id)]);
    assert!(
...
fn a_user_iterates_with_the_agent_over_several_rounds_before_accepting() {
    ...
    let host = commands::capture(&mut store, "# Talk with Arne\n\nrough notes about the roadmap", "").unwrap().id;
    git::commit_all(p, "note: talk with arne", &store.written()).unwrap();
    ...
    let round = |store: &mut FileStore, user: &str, by: Option<(&str, &str)>, agent_says: &str, body: &str| -> String {
        commands::reply(store, &host, user).unwrap();
        commands::reply(store, &host, agent_says).unwrap();
        let prop = commands::create_proposal(store, p, &host, body, &ProposalLimits::default(), by).unwrap();
        git::commit_all(p, "discuss + backup: proposal", &store.written()).unwrap();
        prop.id
    };
    let author_of = |id: &str| -> String {
        String::from_utf8_lossy(&git_run(p, &["log", "-1", "--format=%an", &format!("proposal/{id}")]).stdout)
            .trim()
            .to_string()
    };
    ...
    assert!(d1.patch.contains("## Summary") && !d1.patch.contains("Action items"), "v1 is summary-only: {}", d1.patch);
    decline(&v1);
    ...
    assert_eq!(author_of(&v2), "lfm2.5-1.2b", "round 2 was the phone model — the cycle spans devices");
    ...
    assert!(merge.status.success(), "the accepted round must merge cleanly: {}", String::from_utf8_lossy(&merge.stderr));
    ...
    assert!(live.contains("Arne\n") && !live.contains("Arnne"), "the accepted version fixed the name: {live}");
    ...
    let reread = FileStore::open(p).unwrap();
    assert_eq!(commands::proposals(&reread).unwrap().len(), 3, "all three attempts stay recorded as proposals");
    ...
    assert_eq!(commands::thread(&reread, &host).unwrap().count, 7, "the full back-and-forth is preserved in the note");
}
