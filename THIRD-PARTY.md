# Third-party notices

formicaria's binaries statically link the crates below. Both MIT and Apache-2.0
require their notices to travel with a binary, so they travel here.

The crate table covers **every supported platform**, not just the one this file was
generated on: some crates are reached only on Windows or macOS, and a notice listing
one host's arms would understate what those builds carry.

Generated from the dependency tree by `ci/third-party.sh`; do not edit.

| Crate | Version | Licence |
|---|---|---|
| adler2 | v2.0.1 | 0BSD OR MIT OR Apache-2.0 |
| anstream | v1.0.0 | MIT OR Apache-2.0 |
| anstyle | v1.0.14 | MIT OR Apache-2.0 |
| anstyle-parse | v1.0.0 | MIT OR Apache-2.0 |
| anstyle-query | v1.1.5 | MIT OR Apache-2.0 |
| anstyle-wincon | v3.0.11 | MIT OR Apache-2.0 |
| anyhow | v1.0.103 | MIT OR Apache-2.0 |
| arbitrary | v1.4.2 | MIT OR Apache-2.0 |
| base64 | v0.22.1 | MIT OR Apache-2.0 |
| bitflags | v2.13.0 | MIT OR Apache-2.0 |
| block-buffer | v0.12.1 | MIT OR Apache-2.0 |
| bumpalo | v3.20.3 | MIT OR Apache-2.0 |
| byteorder | v1.5.0 | Unlicense OR MIT |
| cc | v1.2.67 | MIT OR Apache-2.0 |
| cfb | v0.7.3 | MIT |
| cfg-if | v1.0.4 | MIT OR Apache-2.0 |
| clap | v4.6.1 | MIT OR Apache-2.0 |
| clap_builder | v4.6.0 | MIT OR Apache-2.0 |
| clap_derive | v4.6.1 | MIT OR Apache-2.0 |
| clap_lex | v1.1.0 | MIT OR Apache-2.0 |
| colorchoice | v1.0.5 | MIT OR Apache-2.0 |
| const-oid | v0.10.2 | Apache-2.0 OR MIT |
| cpufeatures | v0.3.0 | MIT OR Apache-2.0 |
| crc32fast | v1.5.0 | MIT OR Apache-2.0 |
| crossbeam-utils | v0.8.22 | MIT OR Apache-2.0 |
| crypto-common | v0.2.2 | MIT OR Apache-2.0 |
| deranged | v0.5.8 | MIT OR Apache-2.0 |
| derive_arbitrary | v1.4.2 | MIT OR Apache-2.0 |
| digest | v0.11.3 | MIT OR Apache-2.0 |
| displaydoc | v0.2.6 | MIT OR Apache-2.0 |
| equivalent | v1.0.2 | Apache-2.0 OR MIT |
| errno | v0.3.14 | MIT OR Apache-2.0 |
| fallible-iterator | v0.3.0 | MIT/Apache-2.0 |
| fallible-streaming-iterator | v0.1.9 | MIT/Apache-2.0 |
| fastrand | v2.4.1 | Apache-2.0 OR MIT |
| filetime | v0.2.29 | MIT/Apache-2.0 |
| find-msvc-tools | v0.1.9 | MIT OR Apache-2.0 |
| flate2 | v1.1.9 | MIT OR Apache-2.0 |
| fm-agent | v0.0.0 | MIT |
| fm-agent-run | v0.0.0 | MIT |
| fm-app | v0.0.0 | MIT |
| fm-cli | v0.0.0 | MIT |
| fm-core | v0.0.0 | MIT |
| fm-fetch | v0.0.0 | MIT |
| fm-model | v0.0.0 | MIT |
| fm-query | v0.0.0 | MIT |
| fm-serve | v0.0.0 | MIT |
| fm-update | v0.0.0 | MIT |
| fnv | v1.0.7 | Apache-2.0 / MIT |
| foldhash | v0.2.0 | Zlib |
| form_urlencoded | v1.2.2 | MIT OR Apache-2.0 |
| futures-core | v0.3.32 | MIT OR Apache-2.0 |
| futures-task | v0.3.32 | MIT OR Apache-2.0 |
| futures-util | v0.3.32 | MIT OR Apache-2.0 |
| getrandom | v0.2.17 | MIT OR Apache-2.0 |
| getrandom | v0.3.4 | MIT OR Apache-2.0 |
| getrandom | v0.4.3 | MIT OR Apache-2.0 |
| git2 | v0.21.0 | MIT OR Apache-2.0 |
| hashbrown | v0.16.1 | MIT OR Apache-2.0 |
| hashbrown | v0.17.1 | MIT OR Apache-2.0 |
| hashlink | v0.12.1 | MIT OR Apache-2.0 |
| heck | v0.5.0 | MIT OR Apache-2.0 |
| hybrid-array | v0.4.13 | MIT OR Apache-2.0 |
| icu_collections | v2.2.0 | Unicode-3.0 |
| icu_locale_core | v2.2.0 | Unicode-3.0 |
| icu_normalizer | v2.2.0 | Unicode-3.0 |
| icu_normalizer_data | v2.2.0 | Unicode-3.0 |
| icu_properties | v2.2.0 | Unicode-3.0 |
| icu_properties_data | v2.2.0 | Unicode-3.0 |
| icu_provider | v2.2.0 | Unicode-3.0 |
| idna | v1.1.0 | MIT OR Apache-2.0 |
| idna_adapter | v1.2.2 | Apache-2.0 OR MIT |
| indexmap | v2.14.0 | Apache-2.0 OR MIT |
| infer | v0.19.0 | MIT |
| is_terminal_polyfill | v1.70.2 | MIT OR Apache-2.0 |
| itoa | v1.0.18 | MIT OR Apache-2.0 |
| jobserver | v0.1.35 | MIT OR Apache-2.0 |
| js-sys | v0.3.103 | MIT OR Apache-2.0 |
| libc | v0.2.186 | MIT OR Apache-2.0 |
| libgit2-sys | v0.18.5+1.9.4 | (MIT OR Apache-2.0) AND (GPL-2.0-only WITH linking exception) — vendors libgit2 |
| libsqlite3-sys | v0.38.1 | MIT |
| libz-sys | v1.1.29 | MIT OR Apache-2.0 |
| linux-raw-sys | v0.12.1 | Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT |
| litemap | v0.8.2 | Unicode-3.0 |
| log | v0.4.33 | MIT OR Apache-2.0 |
| memchr | v2.8.3 | Unlicense OR MIT |
| miniz_oxide | v0.8.9 | MIT OR Zlib OR Apache-2.0 |
| num-conv | v0.2.2 | MIT OR Apache-2.0 |
| once_cell | v1.21.4 | MIT OR Apache-2.0 |
| once_cell_polyfill | v1.70.2 | MIT OR Apache-2.0 |
| openssl-probe | v0.1.6 | MIT/Apache-2.0 |
| openssl-src | v300.6.1+3.6.3 | (MIT OR Apache-2.0) AND Apache-2.0 — vendors OpenSSL |
| openssl-sys | v0.9.117 | MIT |
| pem | v3.0.6 | MIT |
| percent-encoding | v2.3.2 | MIT OR Apache-2.0 |
| pin-project-lite | v0.2.17 | Apache-2.0 OR MIT |
| pkg-config | v0.3.33 | MIT OR Apache-2.0 |
| potential_utf | v0.1.5 | Unicode-3.0 |
| powerfmt | v0.2.0 | MIT OR Apache-2.0 |
| ppv-lite86 | v0.2.21 | MIT OR Apache-2.0 |
| proc-macro2 | v1.0.106 | MIT OR Apache-2.0 |
| quote | v1.0.46 | MIT OR Apache-2.0 |
| r-efi | v5.3.0 | MIT OR Apache-2.0 OR LGPL-2.1-or-later |
| r-efi | v6.0.0 | MIT OR Apache-2.0 OR LGPL-2.1-or-later |
| rand | v0.9.5 | MIT OR Apache-2.0 |
| rand_chacha | v0.9.0 | MIT OR Apache-2.0 |
| rand_core | v0.9.5 | MIT OR Apache-2.0 |
| rcgen | v0.13.2 | MIT OR Apache-2.0 |
| ring | v0.17.14 | Apache-2.0 AND ISC |
| rsqlite-vfs | v0.1.1 | MIT |
| rusqlite | v0.40.1 | MIT |
| rustix | v1.1.4 | Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT |
| rustls | v0.23.42 | Apache-2.0 OR ISC OR MIT |
| rustls-pki-types | v1.15.0 | MIT OR Apache-2.0 |
| rustls-webpki | v0.103.13 | ISC |
| rustversion | v1.0.23 | MIT OR Apache-2.0 |
| ryu | v1.0.23 | Apache-2.0 OR BSL-1.0 |
| serde | v1.0.228 | MIT OR Apache-2.0 |
| serde_core | v1.0.228 | MIT OR Apache-2.0 |
| serde_derive | v1.0.228 | MIT OR Apache-2.0 |
| serde_json | v1.0.150 | MIT OR Apache-2.0 |
| serde_yaml_ng | v0.10.0 | MIT |
| sha2 | v0.11.0 | MIT OR Apache-2.0 |
| shlex | v2.0.1 | MIT OR Apache-2.0 |
| simd-adler32 | v0.3.10 | MIT |
| slab | v0.4.12 | MIT |
| smallvec | v1.15.2 | MIT OR Apache-2.0 |
| sqlite-wasm-rs | v0.5.5 | MIT |
| stable_deref_trait | v1.2.1 | MIT OR Apache-2.0 |
| strsim | v0.11.1 | MIT |
| subtle | v2.6.1 | BSD-3-Clause |
| syn | v2.0.118 | MIT OR Apache-2.0 |
| synstructure | v0.13.2 | MIT |
| tar | v0.4.46 | MIT OR Apache-2.0 |
| tempfile | v3.27.0 | MIT OR Apache-2.0 |
| thiserror | v2.0.18 | MIT OR Apache-2.0 |
| thiserror-impl | v2.0.18 | MIT OR Apache-2.0 |
| time | v0.3.53 | MIT OR Apache-2.0 |
| time-core | v0.1.9 | MIT OR Apache-2.0 |
| time-macros | v0.2.31 | MIT OR Apache-2.0 |
| tinystr | v0.8.3 | Unicode-3.0 |
| typenum | v1.20.1 | MIT OR Apache-2.0 |
| ulid | v1.2.1 | MIT |
| unicode-ident | v1.0.24 | (MIT OR Apache-2.0) AND Unicode-3.0 |
| unsafe-libyaml | v0.2.11 | MIT |
| untrusted | v0.9.0 | ISC |
| ureq | v2.12.1 | MIT OR Apache-2.0 |
| url | v2.5.8 | MIT OR Apache-2.0 |
| utf8_iter | v1.0.4 | Apache-2.0 OR MIT |
| utf8parse | v0.2.2 | Apache-2.0 OR MIT |
| uuid | v1.23.5 | Apache-2.0 OR MIT |
| vcpkg | v0.2.15 | MIT/Apache-2.0 |
| wasi | v0.11.1+wasi-snapshot-preview1 | Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT |
| wasip2 | v1.0.4+wasi-0.2.12 | Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT |
| wasm-bindgen | v0.2.126 | MIT OR Apache-2.0 |
| wasm-bindgen-macro | v0.2.126 | MIT OR Apache-2.0 |
| wasm-bindgen-macro-support | v0.2.126 | MIT OR Apache-2.0 |
| wasm-bindgen-shared | v0.2.126 | MIT OR Apache-2.0 |
| web-time | v1.1.0 | MIT OR Apache-2.0 |
| webpki-roots | v0.26.11 | CDLA-Permissive-2.0 |
| webpki-roots | v1.0.9 | CDLA-Permissive-2.0 |
| windows-link | v0.2.1 | MIT OR Apache-2.0 |
| windows-sys | v0.52.0 | MIT OR Apache-2.0 |
| windows-sys | v0.61.2 | MIT OR Apache-2.0 |
| windows-targets | v0.52.6 | MIT OR Apache-2.0 |
| windows_aarch64_gnullvm | v0.52.6 | MIT OR Apache-2.0 |
| windows_aarch64_msvc | v0.52.6 | MIT OR Apache-2.0 |
| windows_i686_gnu | v0.52.6 | MIT OR Apache-2.0 |
| windows_i686_gnullvm | v0.52.6 | MIT OR Apache-2.0 |
| windows_i686_msvc | v0.52.6 | MIT OR Apache-2.0 |
| windows_x86_64_gnu | v0.52.6 | MIT OR Apache-2.0 |
| windows_x86_64_gnullvm | v0.52.6 | MIT OR Apache-2.0 |
| windows_x86_64_msvc | v0.52.6 | MIT OR Apache-2.0 |
| wit-bindgen | v0.57.1 | Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT |
| writeable | v0.6.3 | Unicode-3.0 |
| xattr | v1.6.1 | MIT OR Apache-2.0 |
| yasna | v0.5.2 | MIT OR Apache-2.0 |
| yoke | v0.8.3 | Unicode-3.0 |
| yoke-derive | v0.8.2 | Unicode-3.0 |
| zerocopy | v0.8.54 | BSD-2-Clause OR Apache-2.0 OR MIT |
| zerocopy-derive | v0.8.54 | BSD-2-Clause OR Apache-2.0 OR MIT |
| zerofrom | v0.1.8 | Unicode-3.0 |
| zerofrom-derive | v0.1.7 | Unicode-3.0 |
| zeroize | v1.9.0 | Apache-2.0 OR MIT |
| zerotrie | v0.2.4 | Unicode-3.0 |
| zerovec | v0.11.6 | Unicode-3.0 |
| zerovec-derive | v0.11.3 | Unicode-3.0 |
| zip | v2.4.2 | MIT |
| zmij | v1.0.23 | MIT |
| zopfli | v0.8.3 | Apache-2.0 |

## Bundled into the user interface (npm)

`ui/dist` is compiled into `fm-serve` by `crates/fm-serve/build.rs`, so these
ship inside the binary. Generated from the production dependency closure
(`pnpm licenses list --prod`); build-only tooling is deliberately absent.

| Package | Version | Licence |
|---|---|---|
| @antfu/install-pkg | 1.1.0 | MIT |
| @atlaskit/pragmatic-drag-and-drop | 2.0.1 | Apache-2.0 |
| @babel/runtime | 7.29.7 | MIT |
| @braintree/sanitize-url | 6.0.2, 7.1.2 | MIT |
| @chevrotain/cst-dts-gen | 11.0.3 | Apache-2.0 |
| @chevrotain/gast | 11.0.3 | Apache-2.0 |
| @chevrotain/regexp-to-ast | 11.0.3 | Apache-2.0 |
| @chevrotain/types | 11.0.3, 11.1.2 | Apache-2.0 |
| @chevrotain/utils | 11.0.3 | Apache-2.0 |
| @excalidraw/excalidraw | 0.18.1 | MIT |
| @excalidraw/laser-pointer | 1.3.1 | MIT |
| @excalidraw/markdown-to-text | 0.1.2 | MIT |
| @excalidraw/mermaid-to-excalidraw | 2.2.2 | MIT |
| @excalidraw/random-username | 1.1.0 | MIT |
| @floating-ui/core | 1.8.0 | MIT |
| @floating-ui/dom | 1.8.0 | MIT |
| @floating-ui/react-dom | 2.1.9 | MIT |
| @floating-ui/utils | 0.2.12 | MIT |
| @iconify/types | 2.0.0 | MIT |
| @iconify/utils | 3.1.4 | MIT |
| @jridgewell/gen-mapping | 0.3.13 | MIT |
| @jridgewell/remapping | 2.3.5 | MIT |
| @jridgewell/resolve-uri | 3.1.2 | MIT |
| @jridgewell/sourcemap-codec | 1.5.5 | MIT |
| @jridgewell/trace-mapping | 0.3.31 | MIT |
| @mermaid-js/parser | 0.6.3, 1.2.0 | MIT |
| @radix-ui/primitive | 1.0.0, 1.1.1 | MIT |
| @radix-ui/react-arrow | 1.1.2 | MIT |
| @radix-ui/react-collection | 1.0.1 | MIT |
| @radix-ui/react-compose-refs | 1.0.0, 1.1.1 | MIT |
| @radix-ui/react-context | 1.0.0, 1.1.1 | MIT |
| @radix-ui/react-direction | 1.0.0 | MIT |
| @radix-ui/react-dismissable-layer | 1.1.5 | MIT |
| @radix-ui/react-focus-guards | 1.1.1 | MIT |
| @radix-ui/react-focus-scope | 1.1.2 | MIT |
| @radix-ui/react-id | 1.0.0, 1.1.0 | MIT |
| @radix-ui/react-popover | 1.1.6 | MIT |
| @radix-ui/react-popper | 1.2.2 | MIT |
| @radix-ui/react-portal | 1.1.4 | MIT |
| @radix-ui/react-presence | 1.0.0, 1.1.2 | MIT |
| @radix-ui/react-primitive | 1.0.1, 2.0.2 | MIT |
| @radix-ui/react-roving-focus | 1.0.2 | MIT |
| @radix-ui/react-slot | 1.0.1, 1.1.2 | MIT |
| @radix-ui/react-tabs | 1.0.2 | MIT |
| @radix-ui/react-use-callback-ref | 1.0.0, 1.1.0 | MIT |
| @radix-ui/react-use-controllable-state | 1.0.0, 1.1.0 | MIT |
| @radix-ui/react-use-escape-keydown | 1.1.0 | MIT |
| @radix-ui/react-use-layout-effect | 1.0.0, 1.1.0 | MIT |
| @radix-ui/react-use-rect | 1.1.0 | MIT |
| @radix-ui/react-use-size | 1.1.0 | MIT |
| @radix-ui/rect | 1.1.0 | MIT |
| @sveltejs/acorn-typescript | 1.0.11 | MIT |
| @tauri-apps/api | 2.11.1 | Apache-2.0 OR MIT |
| @types/d3 | 7.4.3 | MIT |
| @types/d3-array | 3.2.2 | MIT |
| @types/d3-axis | 3.0.6 | MIT |
| @types/d3-brush | 3.0.6 | MIT |
| @types/d3-chord | 3.0.6 | MIT |
| @types/d3-color | 3.1.3 | MIT |
| @types/d3-contour | 3.0.6 | MIT |
| @types/d3-delaunay | 6.0.4 | MIT |
| @types/d3-dispatch | 3.0.7 | MIT |
| @types/d3-drag | 3.0.7 | MIT |
| @types/d3-dsv | 3.0.7 | MIT |
| @types/d3-ease | 3.0.2 | MIT |
| @types/d3-fetch | 3.0.7 | MIT |
| @types/d3-force | 3.0.10 | MIT |
| @types/d3-format | 3.0.4 | MIT |
| @types/d3-geo | 3.1.0 | MIT |
| @types/d3-hierarchy | 3.1.7 | MIT |
| @types/d3-interpolate | 3.0.4 | MIT |
| @types/d3-path | 3.1.1 | MIT |
| @types/d3-polygon | 3.0.2 | MIT |
| @types/d3-quadtree | 3.0.6 | MIT |
| @types/d3-random | 3.0.4 | MIT |
| @types/d3-scale | 4.0.9 | MIT |
| @types/d3-scale-chromatic | 3.1.0 | MIT |
| @types/d3-selection | 3.0.11 | MIT |
| @types/d3-shape | 3.1.8 | MIT |
| @types/d3-time | 3.0.4 | MIT |
| @types/d3-time-format | 4.0.3 | MIT |
| @types/d3-timer | 3.0.2 | MIT |
| @types/d3-transition | 3.0.9 | MIT |
| @types/d3-zoom | 3.0.8 | MIT |
| @types/estree | 1.0.9 | MIT |
| @types/geojson | 7946.0.16 | MIT |
| @types/react | 19.2.17 | MIT |
| @types/react-dom | 19.2.3 | MIT |
| @types/trusted-types | 2.0.7 | MIT |
| @upsetjs/venn.js | 2.0.0 | MIT |
| acorn | 8.17.0 | MIT |
| anymatch | 3.1.3 | ISC |
| aria-hidden | 1.2.6 | MIT |
| aria-query | 5.3.1 | Apache-2.0 |
| axobject-query | 4.1.0 | Apache-2.0 |
| binary-extensions | 2.3.0 | MIT |
| bind-event-listener | 3.0.0 | MIT |
| braces | 3.0.3 | MIT |
| browser-fs-access | 0.29.1 | Apache-2.0 |
| canvas-roundrect-polyfill | 0.0.1 | MIT |
| chevrotain | 11.0.3 | Apache-2.0 |
| chevrotain-allstar | 0.3.1 | MIT |
| chokidar | 3.6.0 | MIT |
| clsx | 1.1.1, 2.1.1 | MIT |
| commander | 7.2.0, 8.3.0 | MIT |
| cose-base | 1.0.3, 2.2.0 | MIT |
| crc-32 | 0.3.0 | Apache-2.0 |
| cross-env | 7.0.3 | MIT |
| cross-spawn | 7.0.6 | MIT |
| csstype | 3.2.3 | MIT |
| cytoscape | 3.34.0 | MIT |
| cytoscape-cose-bilkent | 4.1.0 | MIT |
| cytoscape-fcose | 2.2.0 | MIT |
| d3 | 7.9.0 | ISC |
| d3-array | 3.2.4 | ISC |
| d3-array | 2.12.1 | BSD-3-Clause |
| d3-axis | 3.0.0 | ISC |
| d3-brush | 3.0.0 | ISC |
| d3-chord | 3.0.1 | ISC |
| d3-color | 3.1.0 | ISC |
| d3-contour | 4.0.2 | ISC |
| d3-delaunay | 6.0.4 | ISC |
| d3-dispatch | 3.0.1 | ISC |
| d3-drag | 3.0.0 | ISC |
| d3-dsv | 3.0.1 | ISC |
| d3-ease | 3.0.1 | BSD-3-Clause |
| d3-fetch | 3.0.1 | ISC |
| d3-force | 3.0.0 | ISC |
| d3-format | 3.1.2 | ISC |
| d3-geo | 3.1.1 | ISC |
| d3-hierarchy | 3.1.2 | ISC |
| d3-interpolate | 3.0.1 | ISC |
| d3-path | 3.1.0 | ISC |
| d3-path | 1.0.9 | BSD-3-Clause |
| d3-polygon | 3.0.1 | ISC |
| d3-quadtree | 3.0.1 | ISC |
| d3-random | 3.0.1 | ISC |
| d3-sankey | 0.12.3 | BSD-3-Clause |
| d3-scale | 4.0.2 | ISC |
| d3-scale-chromatic | 3.1.0 | ISC |
| d3-selection | 3.0.0 | ISC |
| d3-shape | 3.2.0 | ISC |
| d3-shape | 1.3.7 | BSD-3-Clause |
| d3-time | 3.1.0 | ISC |
| d3-time-format | 4.1.0 | ISC |
| d3-timer | 3.0.1 | ISC |
| d3-transition | 3.0.1 | ISC |
| d3-zoom | 3.0.0 | ISC |
| dagre-d3-es | 7.0.14 | MIT |
| dayjs | 1.11.21 | MIT |
| delaunator | 5.1.0 | ISC |
| detect-node-es | 1.1.0 | MIT |
| devalue | 5.8.1 | MIT |
| dompurify | 3.4.12 | Apache-2.0 (elected from `MPL-2.0 OR Apache-2.0`) |
| es-toolkit | 1.49.0 | MIT |
| es6-promise-pool | 2.5.0 | MIT |
| esm-env | 1.2.2 | MIT |
| esrap | 2.2.13 | MIT |
| fill-range | 7.1.1 | MIT |
| fractional-indexing | 3.2.0 | CC0-1.0 |
| fuzzy | 0.1.3 | MIT |
| get-nonce | 1.0.1 | MIT |
| glob-parent | 5.1.2 | ISC |
| glur | 1.1.2 | MIT |
| hachure-fill | 0.5.2 | MIT |
| iconv-lite | 0.6.3 | MIT |
| image-blob-reduce | 3.0.1 | MIT |
| immutable | 4.3.9 | MIT |
| import-meta-resolve | 4.2.0 | MIT |
| inherits | 2.0.4 | ISC |
| internmap | 1.0.1, 2.0.3 | ISC |
| is-binary-path | 2.1.0 | MIT |
| is-extglob | 2.1.1 | MIT |
| is-glob | 4.0.3 | MIT |
| is-number | 7.0.0 | MIT |
| is-reference | 3.0.3 | MIT |
| isexe | 2.0.0 | ISC |
| jotai | 2.11.0 | MIT |
| jotai-scope | 0.7.2 | MIT |
| katex | 0.16.47, 0.17.0 | MIT |
| khroma | 2.1.0 | MIT (declared in its `license` file, not package.json) |
| langium | 3.3.1 | MIT |
| layout-base | 1.0.2, 2.0.1 | MIT |
| locate-character | 3.0.0 | MIT |
| lodash-es | 4.17.21, 4.18.1 | MIT |
| lodash.debounce | 4.0.8 | MIT |
| lodash.throttle | 4.1.1 | MIT |
| magic-string | 0.30.21 | MIT |
| marked | 16.4.2, 18.0.6 | MIT |
| mermaid | 11.16.0 | MIT |
| multimath | 2.0.0 | MIT |
| nanoid | 3.3.3, 4.0.2 | MIT |
| normalize-path | 3.0.0 | MIT |
| object-assign | 4.1.1 | MIT |
| open-color | 1.9.1 | MIT |
| package-manager-detector | 1.7.0 | MIT |
| pako | 2.0.3 | (MIT AND Zlib) |
| path-data-parser | 0.1.0 | MIT |
| path-key | 3.1.1 | MIT |
| perfect-freehand | 1.2.0 | MIT |
| pica | 7.1.1 | MIT |
| picomatch | 2.3.2 | MIT |
| png-chunk-text | 1.0.0 | MIT |
| png-chunks-encode | 1.0.0 | MIT |
| png-chunks-extract | 1.0.0 | MIT |
| points-on-curve | 0.2.0, 1.0.1 | MIT |
| points-on-path | 0.2.1 | MIT |
| pwacompat | 2.0.17 | Apache-2.0 |
| raf-schd | 4.0.3 | MIT |
| react | 19.2.7 | MIT |
| react-dom | 19.2.7 | MIT |
| react-remove-scroll | 2.7.2 | MIT |
| react-remove-scroll-bar | 2.3.8 | MIT |
| react-style-singleton | 2.2.3 | MIT |
| readdirp | 3.6.0 | MIT |
| robust-predicates | 3.0.3 | Unlicense |
| roughjs | 4.6.4, 4.6.6 | MIT |
| rw | 1.3.3 | BSD-3-Clause |
| safer-buffer | 2.1.2 | MIT |
| sass | 1.51.0 | MIT |
| scheduler | 0.27.0 | MIT |
| shebang-command | 2.0.0 | MIT |
| shebang-regex | 3.0.0 | MIT |
| sliced | 1.0.1 | MIT |
| source-map-js | 1.2.1 | BSD-3-Clause |
| stylis | 4.4.0 | MIT |
| svelte | 5.56.4 | MIT |
| tinyexec | 1.2.4 | MIT |
| to-regex-range | 5.0.1 | MIT |
| ts-dedent | 2.3.0 | MIT |
| tslib | 2.8.1 | 0BSD |
| tunnel-rat | 0.1.2 | MIT |
| use-callback-ref | 1.3.3 | MIT |
| use-sidecar | 1.1.3 | MIT |
| use-sync-external-store | 1.6.0 | MIT |
| uuid | 14.0.1 | MIT |
| vscode-jsonrpc | 8.2.0 | MIT |
| vscode-languageserver | 9.0.1 | MIT |
| vscode-languageserver-protocol | 3.17.5 | MIT |
| vscode-languageserver-textdocument | 1.0.12 | MIT |
| vscode-languageserver-types | 3.17.5 | MIT |
| vscode-uri | 3.0.8 | MIT |
| webworkify | 1.5.0 | MIT |
| which | 2.0.2 | ISC |
| zimmerframe | 1.1.4 | MIT |
| zustand | 4.5.7 | MIT |


## Fonts bundled with the whiteboard

Copied out of `@excalidraw/excalidraw` by `ui/scripts/copy-excalidraw-fonts.mjs` and
baked into the binary, so the whiteboard never fetches a font from the internet.
Xiaolai (CJK) is deliberately **not** bundled — 13 of the 14 MB — so CJK whiteboard
text falls back to a system font.

| Family | Licence | Copyright | Upstream |
|---|---|---|---|
| Excalifont | OFL-1.1 | Excalidraw | <https://plus.excalidraw.com/excalifont> |
| Virgil | OFL-1.1 | Excalidraw (Ellinor Rapp) | <https://github.com/excalidraw/virgil> |
| ComicShanns | **MIT** | Copyright (c) 2018 Shannon Miwa | <https://github.com/shannpersand/comic-shanns> |
| Nunito | OFL-1.1 | Copyright 2014 The Nunito Project Authors | <https://github.com/googlefonts/nunito> |
| Assistant | OFL-1.1 | Copyright 2020 The Assistant Project Authors | <https://github.com/hafontia/Assistant> |
| Lilita One | OFL-1.1 | Copyright (c) 2011 Juan Montoreano, with Reserved Font Name Lilita | <https://fonts.google.com/specimen/Lilita+One> |
| Cascadia Code | OFL-1.1 | Copyright (c) 2019-Present Microsoft Corporation, with Reserved Font Name Cascadia Code | <https://github.com/microsoft/cascadia-code> |
| Liberation Sans | OFL-1.1 | Red Hat, Inc. | <https://github.com/liberationfonts/liberation-fonts> |

OFL-1.1 in full: <https://openfontlicense.org/open-font-license-official-text/>.

## Downloaded at runtime (the optional study assistant)

Not part of these binaries and not present unless the assistant is turned on. Pinned by
URL **and SHA-256** in `models.toml`, fetched once, and kept under this machine's
configuration directory.

| Component | Licence | Source |
|---|---|---|
| llama.cpp (`llama-server` + `ggml`) | MIT | <https://github.com/ggml-org/llama.cpp> |
| whisper.cpp (`whisper-server`) | MIT | <https://github.com/ggml-org/whisper.cpp> |

Model **weights** are downloaded from Hugging Face and carry their own terms, stated in
the app before anything is fetched and recorded per model in `models.toml`.

Full licence texts: <https://spdx.org/licenses/>. formicaria's own licence is MIT,
in `LICENSE`.
