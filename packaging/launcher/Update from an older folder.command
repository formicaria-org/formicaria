#!/bin/sh
# Move your notes into this newer formicaria, on macOS. Double-click this.
#
# **`.command` rather than `.sh`** because that is the extension Finder runs on a double-click —
# the same reason "Start formicaria.command" carries it. A Terminal window opens; that is where
# the questions below are asked and answered.
#
# **Gatekeeper refuses the first double-click** on anything unsigned, exactly as it does for the
# launcher. Right-click -> Open, then "Open anyway". The README covers it in full.
#
# To say which folder to copy from, drag it onto this file in Finder.
#
# See "Update from an older folder.sh" — the logic below is the same, comments and all.
set -eu
