...
| **formicaria on an iPhone / iOS** — before answering "can we", before spending a macOS CI minute | `ios-plan-2026-09-02.md` — the survey, the three settled constraints (no Mac, no iPhone, private repo), the CI ladder with its kill criteria, and why the cheapest answer is already shipped (the LAN share listener) · `decisions.md#track-m` for the three rulings it forced |
