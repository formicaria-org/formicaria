...
|---|---|---|---|
| ~~**B1**~~ **RESOLVED 2026-08-29.** ~~PDFs do not render, and two user-facing strings claim they do.~~ The app's CSP carries `frame-src 'self'` (`fm-serve/src/main.rs:1382`) and a test at `:1988` fails the build if it is ever narrowed. *(The `frame-src 'none'` still in that file at `:1242` is the **manual's** separate, deliberately stricter policy — not this.)* Original finding: `frame-src 'none'` blocks every frame source including `'self'`; the PDF path is an `<iframe>` | `fm-serve/src/main.rs:1168`; `mobile/src-tauri/tauri.conf.json:21` (same); `ui/src/lib/render.ts:385-391`. False claims at `ui/src/lib/SettingsPanel.svelte:699-701` and `packaging/README-release.txt:110-111`. Absent from `known-issues.md`. `render.test.ts:110` asserts the iframe **in jsdom, where no CSP applies** | The exact class `decisions.md:80-92` was written to close. The whole feature starts from a dead viewer |
| ~~**B2**~~ **RESOLVED 2026-08-29.** `edit.rs` gained `"assets" => obj.assets = split_list(raw)` (and the same for `code`), with a comment recording this exact bug. Original finding: `apply_property` has no `assets` arm → falls to `extra` as a scalar `Text` → `from_file`'s `as_string_seq` returns an empty vec on the next load | `fm-core/src/edit.rs:52-57`; `frontmatter.rs:174, 235-240`. `obj.assets` is written in exactly one place: `commands.rs:1294`, by `ingest`, on the asset note it creates | **Silent data loss today.** And it means the plan's central link — paper note → PDF blob — *has no write path at all*. `code` has the same gap |
| **B3** — **mitigated 2026-08-29**, not closed. `kind_pushdown` (`fm-core/src/file.rs:767`) stops planning views hydrating rows a top-level `Kind` excludes, which was the urgent half; the `Prop`/`NoteRef` residual stands, and the fatal case (PDF text in the body) is `outstanding.md` §2.0b. Original finding: `candidates` pushes only `Predicate::Text` to SQL; `Kind`, `Prop`, `NoteRef` are in-memory residuals | `fm-core/src/file.rs:696-701`, `892-915` | Harmless at 1 KB/note prose. Fatal at 2.5 KB/page of PDF text — see Part 3 |
| ~~**B4**~~ **RESOLVED 2026-09-04** — the phone's `fmblob` route now answers a `Range` with 206 + `Content-Range`, advertises `Accept-Ranges`, and carries `nosniff`, a CSP and `Content-Disposition: attachment`; its comment no longer claims to stream, because at that seam it cannot. See `decisions.md#track-m`. Original finding, recorded in `known-issues.md` when it moved there (2026-09-04; it also ships none of `blob.rs`'s security headers, and *"streams"* is unachievable through `register_uri_scheme_protocol`). Original finding: | `mobile/src-tauri/src/lib.rs:233-234` (the comment) vs `:236-247` (the code). Desktop does it correctly at `fm-serve/src/blob.rs` | pdf.js's partial-load design is off on the phone; a 40 MB scan is a 40 MB `Vec` plus wry's copy |
| ~~**B5**~~ **RESOLVED 2026-09-04** — `ingest` and `ingest_finish` both resolve, drop the guard, run the subprocesses, then re-resolve; `fm-app/tests/dispatch_concurrency.rs` holds it with a rendezvous test. Original finding, recorded in `known-issues.md` when it moved there (2026-09-04; `ingest_finish`, added for chunked ingest, inherited it). Original finding: (`pdftotext`, `vipsthumbnail`) | `fm-app/src/dispatch.rs:1618-1620`; `ingest` is absent from the fast list at `:455-478`, and unlike `run_backup` (`:2158-2161`) never drops the guard | A 5,000-PDF import freezes every tab for 25–40 minutes |
| **B6** | **No bundle-size gate exists.** `ci/checks.sh`'s only size budget is on `docs/context/` line count. *(The npm half of this row was closed on 2026-09-03 — see below.)* | `ci/checks.sh:376`, `:127-155` | The eager JS payload today is **~70 KB gz**; one stray top-level import moves 700+ KB into it, green. **Corrected 2026-09-03:** `marked`, `katex`, `mermaid`, `dompurify`, `excalidraw` and `react` are no longer absent from `THIRD-PARTY.md` — `ci/third-party.sh` grew npm and font sections and `pixi run ci` now checks the committed notice for drift. The gap that remains is that the generator reads `ui/` and only `ui/`, so a second app's `package.json` would be invisible to it |
...
3. **Pointing the npm licence gate at the second app** (A7/B6), because `deny.toml` will never see
   pdf.js. **Now an extension rather than a build:** the gate exists as of 2026-09-03
   (`decisions.md`, *The licence notice covers what the binary carries*) and the omission of
   marked/katex/mermaid/dompurify/excalidraw/react is closed — but `ci/third-party.sh` reads `ui/`
   and only `ui/`, so a second app would reopen exactly the gap that one just closed.
