...
| **Backup & acquisition** | ✅ | two-tier: git push (notes) + restic (media, opt-in); clone/restore via `acquire::naturalise`. **Works on Windows without installing git** since 2026-08-28 — libgit2 is linked where the OS ships no binary, and `vcs` still prefers a real one where there is | `decisions.md#git` (Backup, and the Windows widening) |
